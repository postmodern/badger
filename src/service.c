...
ronin_rat_service_t * ronin_rat_service_create(const char *name)
{
	...
	if (!(new_service->functions = ronin_rat_list_create((ronin_rat_list_destroy_func)ronin_rat_func_destroy)))
	{
		goto cleanup_name;
	}
...
{
	return (ronin_rat_func_t *)ronin_rat_list_search(service->functions,name);
}

void ronin_rat_service_destroy(ronin_rat_service_t *service)
{
	ronin_rat_list_destroy(service->functions);
	free(service->name);
	free(service);
}
